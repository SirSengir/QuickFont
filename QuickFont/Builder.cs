...
namespace QuickFont {
    ...
    sealed class Builder {
        string _charSet;
        Dictionary<char, FixedGlyph> _fixedGlyphs;
        QFontBuilderConfiguration _config;
        Font _font;

        public Builder(Font font, QFontBuilderConfiguration config) {
            _charSet = config.CharSet;
            _fixedGlyphs = config.FixedGlyphs;
            _config = config;
            _font = font;
        ...
        private List<SizeF> GetGlyphSizes(Font font) {
            ...
            for (int i = 0; i < _charSet.Length; i++) {
                SizeF charSize;
                if (_fixedGlyphs.ContainsKey(_charSet[i])) {
                    charSize = _fixedGlyphs[_charSet[i]][font.Size].Size;
                } else {
                    charSize = graph.MeasureString("" + _charSet[i], font);
                }
        ...
        private Bitmap CreateInitialBitmap(Font font, SizeF maxSize, int initialMargin, out QFontGlyph[] glyphs, TextGenerationRenderHint renderHint) {
            ...
            Bitmap bmp = new Bitmap(spacing * _charSet.Length, (int)Math.Ceiling(maxSize.Height) + 2 * initialMargin, System.Drawing.Imaging.PixelFormat.Format24bppRgb);
            Graphics graph = Graphics.FromImage(bmp);
            ...
            for (int i = 0; i < _charSet.Length; i++) {
                ...
                SizeF charSize;
                bool suppressColour = false;
                if (_fixedGlyphs.ContainsKey(_charSet[i])) {
                    Bitmap etched = _fixedGlyphs[_charSet[i]][font.Size];
                    charSize = etched.Size;
                    int yOffset = (int)((maxSize.Height - charSize.Height - 2 * initialMargin) / 2);
                    graph.DrawImage(etched, new System.Drawing.Rectangle(xOffset, initialMargin + yOffset, etched.Width, etched.Height), new Rectangle(0, 0, etched.Width, etched.Height), GraphicsUnit.Pixel);
                    suppressColour = true;
                } else {
                    graph.DrawString("" + _charSet[i], font, Brushes.White, xOffset, initialMargin);
                    charSize = graph.MeasureString("" + _charSet[i], font);
                }

                glyphs[i] = new QFontGlyph(0, new Rectangle(xOffset - initialMargin, 0, (int)charSize.Width + initialMargin * 2, (int)charSize.Height + initialMargin * 2), 0, _charSet[i]) { SuppressColouring = suppressColour };
        ...
        public QFontData BuildFontData(string saveName) {
            if (_config.ForcePowerOfTwo && _config.SuperSampleLevels != PowerOfTwo(_config.SuperSampleLevels)) {
            ...
            int pageWidth = _config.PageWidth * _config.SuperSampleLevels; //texture page width
            int pageHeight = _config.PageHeight * _config.SuperSampleLevels; //texture page height
            bool usePowerOfTwo = _config.ForcePowerOfTwo;
            ...
            foreach (QFontGlyph glyph in initialGlyphs) {
                RetargetGlyphRectangleInwards(initialBmp, initialBitmapData, glyph, true, _config.KerningConfig.alphaEmptyPixelTolerance);
            ...
            fontData.KerningPairs = KerningCalculator.CalculateKerning(_charSet.ToCharArray(), glyphs, bitmapPages, _config.KerningConfig);
            ...
            if (_config.ShadowConfig != null)
                fontData.DropShadow = BuildDropShadow(bitmapPages, glyphs, _config.ShadowConfig, _charSet.ToCharArray(), _config.KerningConfig.alphaEmptyPixelTolerance);
        ...
        public static QFontData LoadQFontDataFromFile(QFontData data, IList<Stream> bitmapStreams, float downSampleFactor, QFontLoaderConfiguration loaderConfig) {
            ...
            if (downSampleFactor > 1.0f) {
                ...
                CreateBitmapPerGlyph(ToArray(data.CharSetMapping.Values), bitmapPages.ToArray(), out shrunkGlyphs, out shrunkBitmapsPerGlyph);
            ...
            if (loaderConfig.ShadowConfig != null)
                data.DropShadow = BuildDropShadow(bitmapPages, glyphList.ToArray(), loaderConfig.ShadowConfig, ToArray(data.CharSet), loaderConfig.KerningConfig.alphaEmptyPixelTolerance);

            data.KerningPairs = KerningCalculator.CalculateKerning(ToArray(data.CharSet), glyphList.ToArray(), bitmapPages, loaderConfig.KerningConfig);
        ...
        private static char[] FirstIntercept(Dictionary<char, QFontGlyph> charSet) {
            char[] keys = ToArray(charSet.Keys);
