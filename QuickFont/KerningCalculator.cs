...
namespace QuickFont {
    ...
    sealed class KerningCalculator {
        ...
        public static Dictionary<String, int> CalculateKerning (char[] charSet, QFontGlyph[] glyphs, List<QBitmap> bitmapPages, QFontKerningConfiguration config) {
            ...
            for (int n = 0; n < charSet.Length; n++) {
                ...
                System.Runtime.InteropServices.Marshal.Copy (page.bitmapData.Scan0, marshalled, 0, size);
                ...
                for (int j = yStart; j < yEnd; j++) {
                    ...
                    for (int i = xStart; i < xEnd; i++) {
                        if (!QBitmap.EmptyAlphaPixel (marshalled, page.bitmapData.Stride, i, j, config.alphaEmptyPixelTolerance)) {
